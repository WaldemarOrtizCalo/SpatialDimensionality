RasterCalculations_AreaCorrection<-function(Raster,sample.size,BaseRasterForCorrections){
  tryCatch({# Reproject Raster To UTM
    BaseRaster_Reprojected <- projectRaster(BaseRasterForCorrections,
                                            crs = paste0("+proj=utm +zone=",Auto_UTM_ID(BaseRasterForCorrections)," +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
    ...
    Raster_Reprojected <- projectRaster(Raster,
      ...
      resample(y = BaseRaster_Reprojected,method = "bilinear")
